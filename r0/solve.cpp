...
struct rect_t {
  ...
  int left() const { return pos_.x_; }
  int right() const { return pos_.x_ + w_; }
  int bottom() const { return pos_.y_; }
  ...
  bool include(pos_t const &p) const {
    return left() <= p.x_ && p.x_ < right() && bottom() <= p.y_ && p.y_ < top();
  }
};
